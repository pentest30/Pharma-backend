...
{
    public class CreditNoteToPdfHelper
    {
        private readonly Invoice _invoice;
        private readonly ICommandBus _commandBus;

        public CreditNoteToPdfHelper(Invoice invoice, ICommandBus commandBus)
        {
            _invoice = invoice;
            _commandBus = commandBus;
        }

        public async Task<string> GenerateCreditNotePdfFileAsync()
        {
            ...
            Paragraph invoiceNumber =  new Paragraph($"Avoir N° {_invoice.SequenceNumber}",new Font(Font.NORMAL, 14, Font.BOLDITALIC, BaseColor.BLACK));
