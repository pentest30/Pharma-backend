...
{
    public class ValidateSupplierOrderCommand  : SaveSupplierOrderCommand
    {
    }
    public class ValidateSupplierOrderCommandHandler: ICommandHandler<ValidateSupplierOrderCommand, ValidationResult>
    {
        private readonly IRepository<SupplierOrder, Guid> _supplierOrderRepository;
        private readonly ICurrentOrganization _currentOrganization;
        private readonly ICommandBus _commandBus;

        public ValidateSupplierOrderCommandHandler(IRepository<SupplierOrder, Guid> supplierOrderRepository,
        ...
        public async Task<ValidationResult> Handle(ValidateSupplierOrderCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                order.OrderStatus = ProcurmentOrderStatus.Processing;
                _supplierOrderRepository.Update(order);
                await _supplierOrderRepository.UnitOfWork.SaveChangesAsync();
                var r = await _commandBus.SendAsync(new DeletePendingSupplierOrderCommand { OrderId = request.OrderId }, cancellationToken);
                return r;
